...
/*
    public static boolean setBit(World world, BlockPos block, int x, int y, int z, BlockState state) {
        ...
        if (target_state.isAir()) {
            world.setBlockState(block, ModBlocks.ELECTRONICS_BLOCK.getDefaultState());
        } else if (!target_state.isOf(ModBlocks.ELECTRONICS_BLOCK) && target_state.isFullCube(world, block) && e1 == null) {
            world.setBlockState(block, ModBlocks.ELECTRONICS_BLOCK.getDefaultState(), 0);
            world.addBlockEntity(new ElectronicsBlockEntity(target_state, block, ModBlocks.ELECTRONICS_BLOCK.getDefaultState(), true));
        }
        e1 = world.getBlockEntity(block);
        if (e1 instanceof ElectronicsBlockEntity) {
            ElectronicsBlockEntity e = (ElectronicsBlockEntity) e1;
    ...
    public static boolean canPlace(PlayerEntity player, BlockPos block, int x, int y, int z) {
        ...
        BlockEntity e1 = world.getBlockEntity(block);
        if (e1 instanceof ElectronicsBlockEntity) {
            return ((ElectronicsBlockEntity) e1).getState(x, y, z).isAir();
        }
        return false;
    }
    ...
    public static @Nullable BlockState getBit(World world, BlockPos block, int x, int y, int z) {
        ...
        } else if (!state.isOf(ModBlocks.ELECTRONICS_BLOCK) && canBeChiseled(state, world, block) && state.isFullCube(world, block)) {
            return state;
        }
        return null;
    }
    ...
    public static @Nullable BlockState[][][] getBitArray(World world, BlockPos pos) {
        ...
        if (be instanceof ElectronicsBlockEntity) {
            return ((ElectronicsBlockEntity)be).getStates();
        }
        return null;
    }
    ...
    public static ItemStack getBitItemStack(BlockState state) {
        ItemStack result = new ItemStack(ModBlocks.ELECTRONICS_ITEM);
    ...
    public static void attemptBreak(ServerPlayerEntity player, BlockPos pos, int x, int y, int z) {
        if (canBreak(player, pos, x, y, z)) {
            ...
            BlockState oldstate = ElectronicsUtils.getBit(world, pos, x, y, z);
            if (ElectronicsUtils.exists(oldstate) && ElectronicsUtils.setBit(world, pos, x, y, z, Blocks.AIR.getDefaultState())) {
                ElectronicsUtils.update(world, pos);
                player.getInventory().offerOrDrop(ElectronicsUtils.getBitItemStack(oldstate));
            }
        }
    }
            ...
            for (int j = y1; j <= y2; j++) {
                for (int k = z1; k <= z2; k++) {
                    ...
                    if (canBreak(player, mut, x, y, z)) {
                        BlockState oldstate = ElectronicsUtils.getBit(world, mut, x, y, z);
                        if (ElectronicsUtils.exists(oldstate) && ElectronicsUtils.setBit(world, mut, x, y, z, Blocks.AIR.getDefaultState())) {
        ...
        for (int i = blockx1; i <= blockx2; i++) {
            for (int j = blocky1; j <= blocky2; j++) {
                for (int k = blockz1; k <= blockz2; k++) {
                    mut.set(i, j, k);
                    ElectronicsUtils.update(world, mut);
                }
            }
        }
        for (Object2IntMap.Entry<BlockState> e : drops.object2IntEntrySet()) {
            ItemStack stack = ElectronicsUtils.getBitItemStack(e.getKey());
